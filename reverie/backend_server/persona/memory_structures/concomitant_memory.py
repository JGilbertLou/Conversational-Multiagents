...
class ConcomitantMemory: 
  def __init__(self, f_saved): 
    ...
    self.embeddings = json.load(open(f_saved + "/bert_embedding.json"))
    ...
    for count in range(len(nodes_load.keys())): 
      ...
      #embedding_pair = (node_details["embedding_key"], 
       #                 [self.embeddings[embedding_key] for embedding_key in node_details["embedding_key"]])
      embedding_pair = (node_details["embedding_key"], 
  ...
  def save(self, out_json, c_nodes, c_embedding): 
    r = dict()
    for count in range(len(self.id_to_node.keys()), 0, -1): 
      node_id = f"node_{str(count)}"
      node = c_nodes[node_id]
      ...
      r[node_id]["embedding_key"] = list(node.embedding_key)
      r[node_id]["poignancy"] = node.poignancy
      r[node_id]["keywords"] = list(node.keywords)
      r[node_id]["filling"] = node.filling

    with open(out_json+"/hippo_node.json", "w") as outfile:
      json.dump(r, outfile)

    with open(out_json+"/embedding.json", "w") as outfile:
      json.dump(c_embedding, outfile)

  def save_embedding(self, out_json, c_embedding, file_name):
    with open(out_json+"/" + file_name, "w") as outfile:
      json.dump(c_embedding, outfile)
  ...
  def add_event(self, created, expiration, s, p, o, 
    ...
    node = HippoNode(node_id, node_count, type_count, node_type, depth,
                       created, expiration, 
                       s, p, o, 
    ...
    #for i in range(len(embedding_pair[0])):
    #  self.embeddings[embedding_pair[0][i]] = embedding_pair[1][i]
